...
const cameraSelect = document.querySelector('select#cameraSelect');

vgaButton.onclick = () => { 
  getMedia(constraintsChange(vgaConstraints));
};

qvgaButton.onclick = () => {
  getMedia(constraintsChange(qvgaConstraints));
};

hdButton.onclick = () => {
  getMedia(constraintsChange(hdConstraints));
};

fullHdButton.onclick = () => {
  getMedia(constraintsChange(fullHdConstraints));
};

fourKButton.onclick = () => {
  getMedia(constraintsChange(fourKConstraints));
};

eightKButton.onclick = () => {
  getMedia(constraintsChange(eightKConstraints));
};
...
function constraintsChange(constraints) {
  if (cameraSelect.value == "") {
    return constraints;
  }
  
  var v_width = constraints.video.width.exact;
  var v_height = constraints.video.height.exact;
  
  var newConstraints = {
    video: { width: {exact: v_width}
           , height: {exact: v_height}
